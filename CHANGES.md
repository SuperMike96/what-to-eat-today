...
**修改原因：** 部分菜品 Unsplash 配图与实际菜名不匹配或图片已失效。

**修改内容：** 替换 6 道菜的 Unsplash 照片 ID，全部 20 道菜配图均经过 CDN 可达性验证（HTTP 200）。

| 菜品 | 旧 photo ID | 新 photo ID |
|------|------------|------------|
| 番茄炒蛋 | photo-1562967916 (404) | photo-1625943553852 |
| 肥牛饭 | photo-1504674900247 | photo-1603073163308 |
| 葱油拌面 | photo-1612929633738 | photo-1569718212165 |
| 三杯鸡 | photo-1534939561126 | photo-1598103442097 |
| 咖喱鸡饭 | photo-1601050690597 | photo-1565557623262 |
| 青椒肉丝 | photo-1525755662778 | photo-1546069901 |

**影响文件：** `src/data/dishes.ts`

---

## 2026-07-02 — UI 重构：方案A 单页+底部Tab导航

**修改原因：** 原五步线性向导流程（刷卡→已选→待定→清单→菜谱）交互不够直接，用户被迫按顺序操作。
...
| 改动项 | 说明 |
|--------|------|
| 紧凑顶栏 | 删除 Hero 大标题和说明段落，改为品牌 logo + 行内统计（N已选 · N待定 · N待看） |
| 底部 Tab 栏 | 固定底部：🍳挑选 · 📋菜单 · 🛒清单 · 📖菜谱，菜单 Tab 带红色角标显示已选+待定总数 |
| 待定合并 | 「待定区域」不再独立页面，嵌入「菜单」Tab 底部为可操作分区（加入/移除） |
| 精简按钮 | 撤销/重置改为轻量文字按钮，键盘提示移至卡片下方微弱展示 |
| 步骤导航删除 | 移除顶部 StepNav 组件，Tab 间自由跳转不强制顺序 |
| 类型简化 | `AppStep` 从 `"swipe"|"selected"|"pending"|"shopping"|"recipes"` 改为 `"swipe"|"menu"|"shopping"|"recipes"` |

**影响文件：**
- `src/App.tsx` — 重写（新增 CompactHeader、TabBar、pending-section 组件；删除 StepNav、Hero 文案）
- `src/styles/global.css` — 重写（新增 tab-bar、compact-header、swipe-screen-v2 等样式；删除 hero-copy、step-nav、eyebrow 等旧样式）
- `src/types.ts` — AppStep 类型精简

**功能保留：** 所有原有功能完整保留（刷卡手势、键盘快捷键、撤销重置、购物清单勾选、菜谱滑动、localStorage 持久化）。

---

## 一、评审发现的问题（2026-06-26）
